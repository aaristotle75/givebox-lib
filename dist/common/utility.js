...
export function cleanHtml(html, opts = {}) {
  const defaultOptions = {
    allowedTags: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'p', 'a', 'ul', 'ol', 'nl', 'li', 'b', 'i', 'strong', 'em', 'strike', 'code', 'hr', 'br', 'div', 'table', 'thead', 'caption', 'tbody', 'tr', 'th', 'td', 'pre', 'iframe', 'img', 'span', 'u', 'figure'],
...
export function checkDefault(str) {
  ...
  const default1 = 'akdsfaskdfkdjfkjdk'; //return str !== default1 ? str : '';
...
export function toBinary(accepted) {
  accepted.forEach(file => {
    ...
    reader.onload = () => {
      const fileAsBinaryString = reader.result; // do whatever you want with the file content
...
export const equals = function (array, array2) {
  ...
  if (!array || !array2) return false; // compare lengths - can save a lot of time
...
export function blob2S3(file, s3, fileName, callback, progressCallback) {
...
export function getPublishStatus(kind, webApp) {
  ...
  switch (kind) {
    case 'fundraiser':
      {
        if (webApp) status = 'public';else status = 'private';
        break;
      }

    default:
      {
        if (webApp) status = 'private';else status = 'public';
        break;
      }
  }
