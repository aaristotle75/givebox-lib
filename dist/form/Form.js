...
class Form extends Component {
  ...
  createField(name, args) {
    ...
    if (args.parent) this.fieldProp(args.parent, {
      [name]: args.value
    });
  }
  ...
  fieldProp(name, args) {
    ...
    if (field) {
      ...
      if (field.parent) {
        if (has(params, 'value')) this.fieldProp(field.parent, {
          [name]: params.value
        });
      }
    } else {
  ...
  */
  ...
  validateField(key, validate, value, opts = {}) {
    ...
    switch (validate) {
      case 'custom':
        if (!opts.custom(value)) this.fieldProp(key, {
