// API Resource Actions
export const NEW_REQUEST_RESOURCE = 'NEW_REQUEST_RESOURCE';
export const RELOAD_REQUEST_RESOURCE = 'RELOAD_REQUEST_RESOURCE';
export const RECEIVE_RESOURCE = 'RECEIVE_RESOURCE';
export const RESOURCE_CATCH_ERROR = 'RESOURCE_CATCH_ERROR';
export const SET_RESOURCE_PROP = 'SET_RESOURCE_PROP';
export const REMOVE_RESOURCE = 'REMOVE_RESOURCE'; // API Send Actions

export const SEND_REQUEST = 'SEND_REQUEST';
export const SEND_RESPONSE = 'SEND_RESPONSE'; // Modal Actions

export const TOGGLE_MODAL = 'TOGGLE_MODAL'; // App Actions

export const IS_FILTER_OPEN = 'IS_FILTER_OPEN';
export const SET_APP_REF = 'SET_APP_REF';
export const SET_MODAL_REF = 'SET_MODAL_REF';
export const SET_PROP = 'SET_PROP'; // Preferences

export const SET_PREFERENCES = 'SET_PREFERENCES';