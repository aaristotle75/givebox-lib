...
export function setAccess(res, callback) {
  return (dispatch, getState) => {
    ...
    const status = util.getValue(org, 'status', null); // Set the selected org

    if (orgID) dispatch(resourceProp('orgID', orgID)); // Check if this is a masquerade

    let user;
    if (has(res, 'masker')) user = res.masker;else user = res.user;
    if (user.ID) dispatch(resourceProp('userID', user.ID)); // set access

    const access = { ...util.getValue(getState(), 'resource.access', {}),
      isOwner: false,
